...
- [x] G15: TFT availability is isolated from unrelated source workflows and its freshness limit matches the cadence GitHub actually delivers
  CHECK: node scripts/verify-source-health.mjs
  EXPECT: source health verification passed
  EVIDENCE: 2026-09-06. Isolation holds: availability uses `group: table-for-two-availability-refresh` while the daily writers and the health monitor share `group: source-ledger-refresh`. The original 30-minute half of this gate was unachievable and has been dropped from the title rather than left as a standing false claim: table-for-two-alerts.yml declares `cron: "2,17,32,47 * * * *"`, but across 648 availability commits since 2026-08-05 the median delivered gap is 0.87h, p90 2.01h, max 11.33h, and only 22% are under 30 minutes; source health itself only rebuilds every 30 minutes, so a 0.5h limit could never be met. AVAILABILITY_STALE_HOURS is now 4, covering 97% of observed gaps, and the row reads `current`. The live feed readers actually use is separately guarded at 30 minutes by scripts/check_tft_live_health.py, which fails Monitor Source Health when it lapses.
...
- [x] G17: the primary route starts without waiting for updates or source-health panels, and critical scripts begin downloading from the document head
  CHECK: node scripts/tests/test_initial_load_path.js
  EXPECT: Initial load-path verification passed
  EVIDENCE: exit=0; shell=/bin/sh; cwd=/Users/koohaoming/dev/amex-dining-map; path=db7d51c629d2/23 entries; EXPECT=matched; output-sha256=0d5efb8494f7cb922566f7fae488d0afc37547e2b1bce490008b9e519cd9323d; output-bytes=38

- [x] G18: production TFT loads bounded ratings and release-pattern projections instead of global ratings and raw observations
